...
$.ajax({
  ...
  if (images.length !== 0) {
    ...
    var noImgAvailable = $("<img>").attr("src", "images/noimg.png");
    $(".carousel").append(noImgAvailable);
  }
  ...
  if (park.operatingHours.length !== 0) {
    ...
    $(".operating").text(
      "Hours not provided via API, please visit offical site."
    );
  }
  ...
  var queryUrl2 =
    ...
    "&lon=" +
    long +
    "&units=imperial&exclude=hourly,minutely,alert&appid=510c7f3ff27ad1727021b6aa3db8d1b0";
  $.ajax({
    url: queryUrl2,
    method: "GET",
  }).then(function (response) {
    ...
    for (i = 1; i < 6; i++) {
      $(".date").each(function () {
        ...
        $(".forecastTemp").each(function () {
          ...
          if (i == index) {
            var forecastTemp =
              "Temperature: " + Math.round(response.daily[i].temp.day) + "°";
